...
active_set_newton <- function(Y, A, p, M=400000, init_phi=NULL, diagnostic=FALSE){
    ...
    while (TRUE && safety < 2000) {
        ...
        fn_int = exp(phi[1] + p*log(min(YI_sorted)/sqrt(p)) - (1/2)*log(p))
        ...
        while (new_obj - old_obj < sigma*stepsize * sum(phi_step*grad) - 1e-7){
            ...
            if (nI > 1){
                ...
                fn_int_cand = sum(fnpts_cand * kepler_global_wts) +
                    exp(phi_cand[1] + p * log(min(YI_sorted)/sqrt(p)) - (1/2)*log(p))
            } else {
                fn_int_cand = exp(phi_cand[1] + p * log(min(YI_sorted)/sqrt(p)) - (1/2)*log(p))
            }
